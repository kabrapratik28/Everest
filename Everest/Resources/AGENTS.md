...
| `Artwork/icon-source-alt.png` | `AppIcon-alt.icns` | no, excluded in `project.yml` |

The alt is not dead weight to prune under §1: it was asked for, `project.yml`
keeps it out of the shipped app, and it is the only coverage of the script's
opaque-source alpha path. `Artwork/` is the only copy of the original art —
nothing rebuilds without it, so it is committed, nothing reads from `~/Desktop`,
and paths resolve relative to the script so it runs from any directory.
`project.yml` also excludes `__pycache__`, which xcodegen otherwise copies into
the app the moment someone imports the script rather than running it.
