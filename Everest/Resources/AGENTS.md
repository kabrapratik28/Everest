...
**The alt's source art stays; its 1.5 MB output does not.** The art is the only
coverage of the script's opaque-source alpha path, so pruning it under §1 would
cost a branch nobody else exercises. The `.icns` is just what the script prints
and is regenerated by the same one command, so committing it stored a build
artifact twice over. The `project.yml` exclusion still has to stay even though
the file is not in git: re-run the script and it reappears on disk, and
xcodegen would copy it straight into the bundle. `Artwork/` is the only copy of the original art —
