...
- Ignore the `DVTCoreDeviceCore` plug-in error and `CoreSimulator is out of date` warnings — iOS simulator only, harmless here.
- Compiling from an agent sandbox needs `dangerouslyDisableSandbox: true`.
