...
**Mutation checks run against a copy, never the shared tree.** Breaking code to prove a test catches it is right, and for guards with no honest failing ordering it is the only way. But in a shared checkout it is invisible to you: your runs bracket the mutation, so you see green both sides while anyone building in that window gets a broken artefact and reports failures you cannot reproduce. **And one mutation at a time** — two together can mask each other. Dropping a branch *and* letting an alert inherit it left the alert's test green, because the first removed the very string the second would have leaked; a batch that stays green proves nothing about either half.
...
**Budget: this file ≤150 lines, per-directory ≤60, no exceptions.** Over means cut, not append — a 500-line doc protects nothing because nobody reads it, so the guards get buried and removed anyway. **Or split the directory**, which is what `AppCore` did: it held a ≤90 allowance for carrying four modules' worth of decisions, and moving nine files into `AppCore/Settings/` put both halves under 60 with nothing compressed away. A budget a directory cannot meet is usually telling you the directory is two directories. Nested docs must be named in `Package.swift`'s `exclude:`, or SwiftPM warns once per file per build.

**A rule added here must demand an artefact** — a reason per deleted line, a positive control, a `file:line`, an exit status, a test count. Every clause that caught something on this project asks you to produce one; the ones that ask you to be careful caught nobody, and §8's first wording was read by both people who then broke it. An artefact is checkable by a reader, not only by its author. Rationale, not inventory: "snapshots all types" is useless; "a fixed-delay restore races the user and eats what they copied, so restore is gated on `changeCount`" is the point. Skip anything readable from the code in ten seconds. Update a directory's `AGENTS.md` **and any comment describing the behaviour** in the same change that alters it, then grep for the claim you just falsified: prose outlived the code three times here — "Five refusals" when there were six, "all three fields editable" after two were removed, "still no `keyEquivalent`" after one was added — and each was found by a reader, not by its author. **A documented *measurement* is the dangerous kind** — code cannot contradict it, so no reader can catch it and it ages in silence. "Web password fields do not set `IsSecureEventInputEnabled()`" was false, was the only statement of that fact anywhere, and an external auditor quoted it back as evidence for a P0 that did not exist. Stamp measurements with what was measured and when. Create both files in any new source directory.
