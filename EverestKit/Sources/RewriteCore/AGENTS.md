...
**`clean` removes the model's packaging, never the user's text.** Every rule compares against the source, because that is the only way to tell the two apart, and all three once failed that test. It deleted **every** `<selected_text>` occurrence wherever it appeared, so anyone rewriting prose that mentions the tag lost it from their own sentence. It stripped **any** outer pair of double quotes — which the safety frame tells the model to preserve — so a source holding two quoted phrases came back *unbalanced*: `"A" and "B"` → `A" and "B`; quotes now go only when the source was not quoted too. And a single greedy match across the whole output **spanned two envelopes** when a model restated its input before answering, splicing the original, the commentary and the rewrite into one string. So the envelope is unwrapped only when the id-carrying open and close appear **exactly once** each: the id proves a tag is ours, but not which pair delimits the answer, and leaving the tags in fails visibly where guessing fails silently. **A tidy-up that damages correct input is a worse bug than the tic it was tidying.**
