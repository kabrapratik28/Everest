...
- **`OutputCompleteness`** — source ends a sentence, output does not. Backstop for `AppleFoundationEngine`, which has no stop reason or token count at all, and for an mlx upgrade that stops reporting one. **On MLX it runs only when no stop reason arrives at all.** Running it over a reported `.endOfText` overrides an exact answer with a guess and can only yield false refusals — a real truncation reports `.budgetExhausted` and is caught before it — and it cost every valid rewrite ending in a colon or a list item whose source happened to end as a sentence. Reading `stopReason` was adopted to remove that guesswork; do not layer it back on top. **Not a length ratio, and do not add one:** `Concise` means "significantly shorter", so an honest rewrite is routinely 40% of its source, the proportion a truncation also gives. The signal is where the text stops. Fails open on a fragment source, or refusing every heading and unpunctuated note becomes the new bug.
...
- **Do not single-flight the download the way `LoadOnce` single-flights the load.** Two overlapping `prepare`s do each build a `ModelDownloader`, but `swift-huggingface` already serialises per blob with `flock(2)` and **re-checks the cache inside the lock** (`HubClient+Files.swift:518-537`), so the bytes are fetched once; that is what `.locks/` in `ModelStore.locations(for:)` is. `clearReady`/`markReady` are idempotent and the load behind them is deduped. The waiter retries every second forever via `try await Task.sleep`, so it is cancellable, and on acquisition its progress is set straight to complete. A guard here would duplicate the dependency's, and an unpinned second guard is worse than none.
